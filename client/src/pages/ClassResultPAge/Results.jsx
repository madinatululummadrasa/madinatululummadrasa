...
        const examNames = new Set();
        ...
        // Iterate through all students to find unique years and exam names from detailedResults
        allStudents.forEach(student => {
          const detailedResults = student.detailedResults || {};
          Object.keys(detailedResults).forEach(year => {
            resultYears.add(year);
            Object.keys(detailedResults[year] || {}).forEach(exam => {
              examNames.add(exam);
            });
          });
        });
...
      const subjects = Object.entries(detailedResultData.subjects);
      const total = subjects.reduce((sum, [, mark]) => sum + parseFloat(mark || 0), 0);

      return {
        studentId: student.studentId,
        name: student.name,
        // Use newRoll and originalRoll from detailedResultData
        originalRoll: detailedResultData.originalRoll,
        newRoll: detailedResultData.newRoll,
        subjects,
        total,
      };
    }).filter(Boolean);

    // Sorting is no longer needed here if newRoll is already present and correct in detailedResults
    // If you want to re-rank based on total, keep this line. Otherwise, remove it.
    // resultsWithTotal.sort((a, b) => b.total - a.total);

    // If you removed the sort above, and want to use the newRoll directly from detailedResults
    const rankedResults = resultsWithTotal.map((student) => ({
      ...student,
        // The newRoll is already part of the student object coming from detailedResultData
    }));
...
            <div className="mb-1"><span className="font-semibold">পুরাতন রোল:</span> {student.originalRoll}</div> {/* Changed to originalRoll */}
